...
class PersonalStructuredGraph:
    ...
    def create_structured_demo_graph(self, graph: Graph):
        ...
        topic_node.author = "Topic"

        cat_keep = NodeKnowledge("Evaluation category: KEEP", "KEEP", 40, 90)
        cat_drop = NodeKnowledge("Evaluation category: DROP", "DROP", 40, 150)
        cat_try = NodeKnowledge("Evaluation category: TRY", "TRY", 40, 210)
        for node in (cat_keep, cat_drop, cat_try):
            node.author = "Kategorie"

        per_self = NodeKnowledge("Perspective: Eigene Erfahrung", "Eigene Erfahrung", 260, 90)
        per_peers = NodeKnowledge("Perspective: Mitstudierende", "Mitstudierende", 260, 150)
        per_research = NodeKnowledge("Perspective: Recherche", "Recherche", 260, 210)
        ...
        for title, description_text, category, perspective, source in knowledge_nodes:
            ...
            if category == cat_keep:
                pos_x, pos_y = next(keep_positions)
            elif category == cat_drop:
    ...
    def _extend_description(self, title, description_text):
        guest_lecture_core = (
            ...
            "Was sollen Studierende danach koennen, und welches Fallbeispiel zeigt das? "
            "Ein reales Projekt ist dann hilfreich, wenn es die Schritte zeigt, die im Studium "
            "oft nur abstrakt vorkommen: Anforderungen aus Stakeholder-Gespraechen, Begruendung "
            "von Architekturentscheidungen, Umgang mit technischen Schulden und die Rolle von Tests. "
            "Auch Details wie Datenqualitaet, Wartung oder Sicherheitsbedenken zeigen, warum "
            "theoretische Inhalte gebraucht werden."
        )
