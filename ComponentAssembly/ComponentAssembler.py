...
from View.UI.UIThemes import BKTheme, GISTheme, LightBlueTheme, LightBlueThemeWhiteNodeDetails
...
class ComponentAssembler(IComponentProvider):
    ...
    #uiTheme = LightBlueThemeWhiteNodeDetails()
    #uiTheme = GISTheme()
    ...
    def __init__(self, graph: Graph, screen, window_width, window_height):
        ...
        self.scale_offset_transformer = ScaleOffsetTransformer(self.window_width, self.window_height, self.application_settings)

        # Calculate Similarity
        # self.SIMILARITY_Threshold = 0.9979  # Good Value around 0.998
        self.SIMILARITY_Threshold = 0.8  # Good Value around 0.998
        if self.application_settings.allow_calculation_of_similarity:
            # Führe den Import nur dann aus, wenn die Berechnung der Ähnlichkeit erlaubt ist
            ...
            # self.similarity_calculator = BERTSimilarityCalculator(graph, self.SIMILARITY_Threshold)
            self.similarity_calculator = MiniLMSimilarityCalculator(graph, self.SIMILARITY_Threshold)
        ...
        self.graph_visualizer = GraphVisualizer(self.window_width, self.scale_offset_transformer,
