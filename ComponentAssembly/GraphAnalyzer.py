...
class GraphAnalyzer:
    ...
    def __count_nodes(self, graph):
        """
        Counts the number of nodes that are exactly of type NodeKnowledge in the graph.
        """
        return sum(1 for node in graph.nodes if type(node) is NodeKnowledge)
    ...
    def __count_characters(self, graph):
        """
        Counts the total number of characters in the description and title of each node that is exactly of type NodeKnowledge.
        """
        total_characters = 0
        for node in graph.nodes:
