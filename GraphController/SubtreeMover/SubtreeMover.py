...
class SubtreeMover:
    ...
    def move_selected_node(self, node_to_move, dx, dy):
        """
            Moves a selected node by a specified offset without affecting its subtree.

            :param: node_to_move: The node to be moved.
            :param: dx: The change in the x-coordinate for the movement.
            :param: dy: The change in the y-coordinate for the movement.

            This method updates the position of the specified node only, without translating its connected nodes.
            """
        if node_to_move is None:
            return
