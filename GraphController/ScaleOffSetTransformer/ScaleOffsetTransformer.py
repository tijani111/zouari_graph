...
from GraphController.ScaleOffSetTransformer.IScaleOffsetTransformer import IScaleOffsetTransformer
from GraphController.ScaleOffSetTransformer.IScaleOffsetTransformerObservable import IScaleOffsetTransformerObservable
from GraphController.ScaleOffSetTransformer.IScaleOffsetTransformerObserver import IScaleOffsetTransformerObserver


class ScaleOffsetTransformer(IScaleOffsetTransformer, IScaleOffsetTransformerObservable):
    """
    ...
    def __init__(self, window_width, window_height, application_settings: ApplicationSettings):
        """
        ...
        """
        self.start_x = application_settings.start_x
        self.start_y = application_settings.start_y
        self.zoom = application_settings.zoom
        self.zoom_scaling_factor = self.zoom * 0.1
        self.max_zoom_out = 0.0000001

        # Offset wird auf Start position initalisiert
        self.offset_x = window_width // 2 - int(self.start_x * self.zoom)
        self.offset_y = window_height // 2 - int(self.start_y * self.zoom)
        self.observers = []
    ...
    def subscribe(self, observer: IScaleOffsetTransformerObserver):
