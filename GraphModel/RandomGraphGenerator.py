...
class RandomGraphGenerator:
    ...
    def create_random_hierarchical_graph(self, amount_of_nodes):
        ...
        """
        node_stack = Queue()
        root_node = self.__create_random_node_knowledge(self.test_text, "Titel of this Node", "image_placeholder.png")
    ...
    def __create_random_hierarchical_graph_children(self,
        ...
        for i in range(amount_of_children):
            ...
            node = self.__create_random_node_knowledge(self.test_text, "Titel of this Node", "image_placeholder.png")
    ...
    def create_random_nodes(self, amount_of_nodes, number_of_connections):
        ...
        # 10% of the total amount of nodes for special types
        num_special_nodes = max(1, amount_of_nodes // 10)

        # Create special nodes
        self.__create_random_illustration_nodes(num_special_nodes)
        self.__create_random_source_book_nodes(num_special_nodes)
        self.__create_random_source_online_nodes(num_special_nodes)
        self.__create_random_source_paper_nodes(num_special_nodes)

        remaining_nodes = amount_of_nodes - 4 * num_special_nodes

        # Random distribution force directed graphs will not work if all Nodes are on the same position
        for i in range(remaining_nodes):
            ...
            self.__create_random_node_knowledge(self.test_text, "Titel of this Node")

        self.__random_connect_nodes(number_of_connections)

        # self.fruchterman_reingold_layout(self.graph, 10000, 10000, 10000)

    def __create_random_illustration_nodes(self, count):
        for _ in range(count):
            node = NodeIllustration("Illustration Node", "image_placeholder.png")
            self.graph.add_new_node_to_graph(node)

    def __create_random_source_book_nodes(self, count):
        for _ in range(count):
            node = NodeSourceBook(
                "Titel des Buches", "Autor des Buches", "2023",
                "Verlagsort", "Verlag XY", "123-456-789"
            )
            self.graph.add_new_node_to_graph(node)

    def __create_random_source_online_nodes(self, count):
        for _ in range(count):
            node = NodeSourceOnline(
                "Titel des Dokuments", "Autor XYZ", "2023",
                "Website Titel", "https://example.com", "01.01.2023"
            )
            self.graph.add_new_node_to_graph(node)

    def __create_random_source_paper_nodes(self, count):
        for _ in range(count):
            node = NodeSourcePaper(
                "Titel des Artikels", "Autor ABC", "2023",
                "Journal Name", "Vol 1, Issue 1"
            )
            self.graph.add_new_node_to_graph(node)

    def __create_random_node_knowledge(self, description, titel):
        """
        ...
        """
        node = NodeKnowledge(description, titel)
        self.graph.add_new_node_to_graph(node)
        return node
